...
5. **タスクIDを必ず使う(2026-09-13追加、ユーザー指示)**。出力の各行の先頭の
   通し番号(1, 2, 3...)が、そのタスクのID(タスクID)になる。タスクIDは常に
   `【タスクID:N】`という固定の書式(Nは番号)で本文中に埋め込むこと。この
   固定書式は、拡張機能が「同じタスクIDの出現箇所を順にジャンプする」機能で
   ページ内テキストを検索するために使うため、書式を変えてはいけない。
   - 新しく検出したタスクは、`内容`の要約文の**語尾にも`【タスクID:N】`を
     入れる**(例: `内容: findSendButton()の修正が未完了 【タスクID:1】`)。
   - この監査の後、同じルーム内で特定のタスクについてやり取り(進捗報告・
     質問・修正の相談など)する場合、**その回答の先頭に必ず`【タスクID:N】`を
     付ける**(例: `【タスクID:3】findSendButton()の修正を完了しました。`)。
