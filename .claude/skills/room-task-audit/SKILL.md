...
5. **タスクIDを必ず使う(2026-09-13追加、2026-09-25書式変更、ユーザー指示)**。
   出力の各行の先頭の通し番号(1, 2, 3...)は、あくまで**表示順の位置**であり、
   タスクIDそのものではない(この番号は次回の監査で1から振り直されるため)。
   タスクIDの実体は、`【タスクID:回答MM/DD HH:MM:SS-N】`という書式で本文中に
   埋め込む。`MM/DD HH:MM:SS`はこの回答自身の`【回答MM/DD HH:MM:SS】`(手順3参照)
   と**必ず同じ値**を使い、`N`はこの監査出力内での行番号(1, 2, 3...)。
   この固定書式は、拡張機能が「同じタスクIDの出現箇所を順にジャンプする」機能で
   ページ内テキストを検索するために使うため、書式を変えてはいけない
   (拡張機能側はこの文字列全体をそのままパースして検索キーにするため、`回答`の
   部分も省略しないこと)。
   - 新しく検出したタスクは、`内容`の要約文の**語尾にも`【タスクID:回答MM/DD HH:MM:SS-N】`を
   ...
   - この監査の後、同じルーム内で特定のタスクについてやり取り(進捗報告・
     ...
     ではなく、対応するタスクの`【タスクID:回答MM/DD HH:MM:SS-N】`(監査時点のもの、
     元の値のまま)を付ける**(例: `【タスクID:回答09/25 16:47:03-3】findSendButton()の修正を完了しました。`)。
