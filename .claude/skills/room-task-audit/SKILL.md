...
5. **タスク番号を必ず使う(2026-09-13追加、ユーザー指示)**。出力の各行の先頭の
   通し番号(1, 2, 3...)が、そのタスクの識別番号になる。
   - 新しく検出したタスクは、`内容`の要約文の**語尾にもこの番号を入れる**
     (例: `内容: findSendButton()の修正が未完了(1)`)。表形式の一覧で番号が
     見えない場合でも、要約文自体から番号を追えるようにするため。
   - この監査の後、同じルーム内で特定のタスクについてやり取り(進捗報告・
     質問・修正の相談など)する場合、**その回答の先頭に必ず該当のタスク番号を
     付ける**(例: `3. findSendButton()の修正を完了しました。`)。番号を
     見た人がどのタスクの話か即座に分かるようにするため。
