...
3. **`create_trigger`を呼ぶ**。`create_new_session_on_fire: true`、`persistent_session_id`は指定しない。promptには以下を明記する(claude-voice-bridge向けに作った実例をテンプレートとして使ってよい):
   ...
   - **「対象が0件」と「そもそも読み込めなかった」を明確に区別する**(2026-09-19追加、ユーザー指摘「ルーティンで読み取りできなければ、エラーとして報告するようにして」)。リポジトリのclone失敗、JSONデータ1・2のファイルが存在しない/壊れている等で読めない場合は、0件扱いで静かに終了せず、どの手順で何のエラーが起きたかを具体的に報告して処理を中断する(この場合commit不要)
   ...
   - 完了報告は簡潔にする(詳細な説明は不要、と明記しておかないと長くなりがち)。ただし**正常終了時は必ず処理件数(sourceCount相当)を、エラー終了時は具体的な原因を報告するよう明記する**
