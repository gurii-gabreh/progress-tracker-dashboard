...
- **スプレッドシート「実装進捗管理シート」が唯一の正本**
  ...
  - **ナレッジタブ**(2026-08-12追加): `Repo, Task, タスクID, ステータス, 更新日, 実装ナレッジ, 備考, 問題点, 成果物`。完了タブとは異なり**ステータスを問わず全タスクを機械的にミラーする**(運用ルール35参照)
...
34. **ユーザーからの指示における「親タスク」「子タスク」の呼称を明確化した**(2026-08-11追加、
    ...
    - ユーザーが「親タスクとして追加して、子タスクとして実装内容を明記して」のように依頼した場合、
      ...
      別物であることに注意する
35. **スプレッドシートに「ナレッジタブ」を新設し、ステータスを問わず全タスクの`detail`(実装ナレッジ)を
    機械的にミラーするようにした**(2026-08-12追加、マネージャールームでの相談から)。
    ...
    - **仕組み**: `gas/Code.gs`の`syncFromGithub()`(運用ルール32と同じPull型同期、時間主導トリガーで
      ...
      ステータスに関わらず全タスクを`ナレッジ`タブへ1行ずつupsertする。要約や取捨選択は一切行わず、
      `data/tasks.json`の`note`/`detail`/`issues`/`output`をそのまま転記するだけなので、空欄なら
      空欄のまま書き込まれる。「何が書かれていないか」自体がスプレッドシート単体を見るだけで
      機械的に分かることを目的とした設計
