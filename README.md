...
20. **ユーザー作業タスク(`category: "manual"`)の子タスクなし・末端ノードには、ダッシュボード上に
    「✓ 完了にする」ボタンが出る**(2026-08-04追加)。押すとブラウザから直接GASの`completeTasks`を
    呼び、依頼タスクタブからの削除・完了タブへの追記まで(ユーザーの手を離れて)その場で行われる。
    Routine側は、次回実行時に`data/tasks.json`上でこのタスクが既に`status: 完了`になっていることを
    見て、通常の完了タスクと同様に扱ってよい(重複してcompleteTasksを呼んでも実害はない。GAS側は
    同じrepo+taskの行があれば上書きする実装のため)
