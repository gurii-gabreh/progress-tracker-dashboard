...
34. **ユーザーからの指示における「親タスク」「子タスク」の呼称を明確化した**(2026-08-11追加)。
    - **親タスク** = レベル1タスク(`相談事項`の各テーマや、kizashi「結」のようにリポジトリ直下に
      並ぶトップレベルのタスクと同じ階層)
    - **子タスク** = その親タスクの`subtasks`配列に入る、直下のタスク(運用ルール13の`subtasks`と
      同じもの)
    - ユーザーが「親タスクとして追加して、子タスクとして実装内容を明記して」のように依頼した場合、
      上記の対応関係でdata/tasks.jsonへ反映する(新しい概念ではなく、既存の運用ルール13・24の
      粒度をどう呼ぶかの用語合わせ)
