...
34. **ユーザーからの指示における「親タスク」「子タスク」の呼称を明確化した**(2026-08-11追加、
    同日中に一度誤って実装し、ユーザー指摘を受けて訂正)。
    - **親タスク** = `相談事項`(PTD-019)やkizashi「結」と同じレベルのタスク。具体的には、
      `data/tasks.json`の`tasks`配列に**フラットに並ぶ、トップレベルの1エントリ**を指す
      (`subtasks`配列にネストされていない)
    - **子タスク** = その親タスクの**「レベル1タスク」**、すなわち`subtasks`配列にネストするの
      ではなく、**同じ`repo`値を持つ、別のフラットなトップレベルエントリとして並べる**タスクを
      指す。実例: `相談事項`グループでは、PTD-019(親、テーマ集のタイトル)に対し、
      PTD-020〜036が全て`"repo": "相談事項"`を持つフラットな並列エントリとして並んでおり、
      `subtasks`には一切ネストされていない。これが「子タスク=親タスクのレベル1タスク」の意味
    - **誤りやすい点(2026-08-11に実際に間違えた例)**: 「PTD-020を親タスク化し、子タスクを
      追加して」と言われた際、PTD-020の`subtasks`配列に新タスクをネストしてしまうと、
      ダッシュボード上ではLv2(孫タスク相当の入れ子)になってしまい、意図とは異なる。正しくは、
      新タスクをPTD-020と同じ`repo`値を持つ**別のフラットなトップレベルエントリ**として追加する
